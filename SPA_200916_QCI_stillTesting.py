...
#--------Script params---------------------------
design_name = 'SPA_200917_QCI_TestTopQuarter' #this is the name of the gds file that gets output
DoseTestBool = 1
...
#################################Testing stuff #############################################################
resCell.copy(name='resCell_rot',deep_copy=False,translation=None,rotation=np.pi/2)
...
if SPABool:    
    ...
    chipCell.add(oldcellSnailArray)
    
    
    #sets up top level array and postions 
    TopLevelNx=1 #number of chip slots in final array 
    TopLevelNy=1 
    
    Offset=[0*mm,0*mm] #offset from the center of the wafer 
    
    StartX=Offset[0]-((TopLevelNx-1)*chipx)/2
    StartY=Offset[1]-((TopLevelNy-1)*chipy)/2
    
    locx = np.linspace(StartX, StartX+(TopLevelNx-1)*chipx, TopLevelNx)
    locy = np.linspace(StartY, StartY+(TopLevelNy-1)*chipy, TopLevelNy)
    ...
    for loc in SPA_locs:
        #print(loc)
        topCell.add(gdspy.CellReference(chipCell, loc))
